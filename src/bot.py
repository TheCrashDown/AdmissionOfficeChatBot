...
def monitoring(chat_id):
    ...
    stats = ("У вас баллов: {0}\n"
             ...
             "Число бюджетных мест в этом году: 95").format(your_summary, above, above_, number_of_people)
...
def set_mail(message):
    try:
        if data_base_telegram.set_email(message.chat.id, message.text):
            bot.send_message(message.chat.id, "Пользователя с такой электроной почтой нет в базе данных приемной комисси\n"
                                              "Для тестирование работы бота вы можете использовать почту q@q")
            return
        data_base_telegram.set_status(message.chat.id, "")
        monitoring(message.chat.id)
    except Exception:
